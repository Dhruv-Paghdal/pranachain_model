        ...
        if file_path.exists():
            try:
                ...
                if 'SEQN' in df.columns and len(df) > 0:
                    ...
                    print(f"⚠ Skipping {filename}: No SEQN column or empty file")
...
class DiabetesPredictionDataProcessor:
    ...
    def load_and_process_datasets(self, data_path: str = '/content/diabetes_csvs/') -> pd.DataFrame:
        ...
        print("🔍 Loading and processing diabetes-relevant datasets...")
            ...
            for file_path in glob.glob(search_pattern):
                try:
                    ...
                    if len(available_features) > 1:
                        ...
                        print(f"✅ Loaded {os.path.basename(file_path)}: {len(df)} rows, {len(available_features)} features")
                except Exception as e:
        ...
        print(f"\n📊 Final merged dataset: {len(consolidated_data)} rows, {len(consolidated_data.columns)} features")
        return consolidated_data


def main():
    print("🚀 Creating single CSV with relevant features only...")
...
class DiabetesPredictionDataProcessor:
    ...
    def load_and_process_datasets_in_batches(self, data_path: str = '/content/diabetes_csvs/', batch_size: int = 100, batch_save_path: str = '/content/batches/') -> None:
        ...
        print("🔍 Loading and processing datasets in batches...")
        ...
        for i in range(0, len(all_files), batch_size):
            ...
            for file_path in batch_files:
                try:
                    ...
                    if 'SEQN' not in df.columns:
                        print(f"⚠️ Skipping {os.path.basename(file_path)} (no SEQN column)")
                        continue
            ...
            if batch_consolidated is not None:
                ...
                print(f"💾 Saved merged batch as {batch_file}")

def main():
    print("🚀 Processing all datasets in batches and saving merged batch files...")
